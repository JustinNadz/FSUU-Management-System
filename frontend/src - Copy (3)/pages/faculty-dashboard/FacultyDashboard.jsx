    ...
    <div className="h-screen bg-background flex relative">
      ...
      <div className="flex-1 flex flex-col min-w-0 h-screen overflow-y-auto">
          ...
          <div className="relative profile-menu-container">
            ...
            >
              <div className="w-8 h-8 sm:w-9 sm:h-9 rounded-full bg-green-600 text-white grid place-items-center text-xs font-medium overflow-hidden">
                {(previewUrl || avatarUrl) ? <img src={previewUrl || avatarUrl} alt="" className="w-full h-full object-cover" /> : <span>{user?.name?.[0] || 'F'}</span>}
              </div>
        ...
        <main className="p-2 sm:p-4 md:p-6 space-y-6">
          ...
          {/* Students section removed per request */}
          {active==='profile' && (
            ...
            <div className="bg-white border border-border rounded-lg p-4 sm:p-6">
              ...
              <div className="space-y-4 max-w-sm">
                <div>
                  ...
                  <input type="password" className="w-full h-9 px-3 border rounded text-xs" value={pwdCurrent} onChange={e=>setPwdCurrent(e.target.value)} />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">New Password</label>
                  <input type="password" className="w-full h-9 px-3 border rounded text-xs" value={pwdNew} onChange={e=>setPwdNew(e.target.value)} />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Confirm New Password</label>
                  <input type="password" className="w-full h-9 px-3 border rounded text-xs" value={pwdConfirm} onChange={e=>setPwdConfirm(e.target.value)} />
                </div>
                <div className="flex gap-2 pt-4">
                  <button onClick={()=>setActive('profile')} className="h-9 px-4 bg-gray-600 text-white rounded text-xs hover:bg-gray-700 transition-colors">Cancel</button>
                  <button
                    onClick={async ()=>{
                      try {
                        setPwdSaving(true)
...
function FacultyProfile({ user, onLogout }) {
  ...
  // Removed editable department/course fields per request
  const update = (k,v) => setProfile(p => ({ ...p, [k]: v }))
  // Removed fetch/populate for department/course per request
  const save = async () => {
    try {
      await api.put('/profile/me', { name: profile.name, email: profile.email })
      const stored = JSON.parse(localStorage.getItem('auth') || 'null')
      if (stored?.user) { stored.user.name = profile.name; stored.user.email = profile.email }
      else if (stored) { stored.name = profile.name; stored.email = profile.email }
  ...
  return (
    <div className="space-y-4 max-w-sm">
      <div>
        <label className="block text-[11px] font-medium mb-1">Avatar</label>
        <div className="flex items-center gap-3">
          <div className="w-14 h-14 rounded-full overflow-hidden bg-gray-200">
            {(previewUrl || user?.avatar_url) ? <img src={previewUrl || user?.avatar_url} alt="" className="w-full h-full object-cover" /> : null}
          </div>
      ...
      <div>
        ...
        <input disabled value={user?.employee_no || ''} className="w-full h-9 px-3 border rounded text-xs bg-gray-50" />
      </div>
